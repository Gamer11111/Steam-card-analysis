   ...
   static Logger log = Logger.getLogger(Analysis.class.getName());
    ...
    new FileWriter("Profit.txt",true)
            ...
            for (String i : appid) {
                ...
                index = appid.indexOf(i);
                if (Double.parseDouble(setPrice.get(index).replace(",", "")) >= price) {
   ...
   static public int getPriceHistory(String currentAppid) throws Exception   {
        ...
        for (String i : singleGame) {
           try{
            ...
            String pricehistory = Jsoup.connect(priceLink + currentAppid + "-" + i).cookie("steamLoginSecure", cookie).ignoreContentType(true).execute().body();
